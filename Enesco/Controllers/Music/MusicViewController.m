...
@interface MusicViewController ()<MusicListViewControllerDelegate, PKDownloadButtonDelegate>
...
- (IBAction)menuAction:(id)sender {
    ...
    MusicListViewController *listVC = [storyboard instantiateInitialViewController];
    listVC.delegate = self;
    listVC.musicEntities = _musicEntities;
...
- (void)setupMusicViewWithMusicEntity:(MusicEntity *)entity {
    MusicEntity *ent =[[LocalManager manager]getLocalMusicWithURL:entity.musicUrl];
    if (ent) {
        _musicEntity = ent;
    }else {
        _musicEntity = entity;
...
{
    __weak typeof(self) weakSelf = self;
    [[ZFDownloadManager sharedInstance] download:_musicEntity.musicUrl progress:^(CGFloat progress, NSString *speed, NSString *remainingTime, NSString *writtenSize, NSString *totalSize) {
        dispatch_async(dispatch_get_main_queue(),^{
            weakSelf.downloadButton.stopDownloadButton.progress = progress;
        });
    } state:^(DownloadState state) {
        dispatch_async(dispatch_get_main_queue(),^{
            switch (state) {
                case DownloadStateStart:
                    if (weakSelf.downloadButton.state != kPKDownloadButtonState_Downloading) {
                        [[LocalManager manager] addMusic:_musicEntity];
                        weakSelf.downloadButton.state = kPKDownloadButtonState_Downloading;
                    }
                    break;
                ...
                case DownloadStateCompleted:
                    ...
                    [[LocalManager manager] setDownloadedWithMusicURL:_musicEntity.musicUrl];
                    break;
...
- (void)resetDownloadButton {
    ...
    //  如果下载完成
    if (_musicEntity.isDownloaded) {
        self.downloadButton.state = kPKDownloadButtonState_Downloaded;
    }else {
        self.downloadButton.stopDownloadButton.progress = [[ZFDownloadManager sharedInstance] progress:_musicEntity.musicUrl];
    }
}
...
- (void)createStreamer {
    [[MusicAPI defaultManager]checkNetworkWithBlock:^(bool isWork) {
        if (isWork) {
            if (_specialIndex != -1) {
                if (_specialIndex == _currentIndex) {
                    return;
                }
            ...
            if (_musicEntity.isDownloaded) {
                track.audioFileURL = [NSURL fileURLWithPath:_musicEntity.localPath];
            }else {
